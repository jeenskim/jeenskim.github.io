...
spack:
  ...
  specs:
  - fenics-dolfinx+adios2 ^petsc+mumps  #Build petsc library with mumps option
  - py-fenics-dolfinx cflags=-O3 fflags=-O3
  - py-torch+cuda cuda_arch=80 #Setting GPU architecture
  - py-pip
  - gmsh
  - adios2+python # For using `adios4dolfinx` for writing/reading checkpoints
  view: true
  concretizer:
    unify: true
```
...
#### 1.4. Installing python packages using pip

##### (NOTE) MUST switch to an environment with GPU access (e.g., connect to a compute node) because pip command depends on hardware settings.
